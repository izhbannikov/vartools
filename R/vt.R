vt_method <- function(casecon, gen, mafs, h.maf, maf) {
    ...
    for (i in 1:(length(h.maf)-1)) {
      ...
      z.num <- sum(gen[,mafs<maf] * y.new, na.rm=TRUE)
      z.denom <- sqrt(sum((gen[,mafs<maf])^2, na.rm=TRUE))
      z.scores[i] <- z.num / z.denom
    }
    stat <- max(z.scores,na.rm = T)
    
    stat
}

vt <- function(table, maf=0.05, perm=1) {
    ...
    vt.stat <- vt_method(y, X, mafs, h.maf, maf)
    ...
    if (perm > 0)
    {
      ...
      for (i in 1:perm)
      {
        ...
        x.perm[i] <- vt_method(y[perm.sample], X, mafs, h.maf, maf)
      }
      ## p-value
