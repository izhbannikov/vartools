rbt_method <- function(casecon, gen) {
    ...
    fA <- colSums(as.matrix(gen[which(casecon==1),]), na.rm=TRUE)
    fU <- colSums(as.matrix(gen[which(casecon==0),]), na.rm=TRUE)
    ...
    if (sum(AltU) > 0)
    {
      ...
      if (length(uniqA.plus) == 1)
      {
        ...
        for (j in 1:ncolsA.plus) {
          for (i in kU.plus[uniqA.plus[j]==kA.plus]) {
      ...
      if (length(uniqA.minus) == 1)
      {
        ...
        for (j in 1:ncolsA.minus) {
          for (i in kU.minus[kA.minus==uniqA.minus[j]]) {
            NUA.minus[which(uniqU.minus==i),j] <- sum(kU.minus[which(kA.minus==uniqA.minus[j])] == i)
          }
        }
